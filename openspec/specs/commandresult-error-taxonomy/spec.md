...
### REQ-10 — `PersonaSkillCommandResult` y `PersonaSkillError` en `SGV.Contracts.Personas`
...
### REQ-11 — Mapeo `PersonaSkillErrorType → ErrorCategoria` consolidado
...
### REQ-12 — `PersonaSkill` no reintroduce un enum paralelo
...
#### Scenario: `HabilidadError` con `CategoriaInexistente` se traduce a HTTP 400
