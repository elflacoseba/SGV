...
- El endpoint `GET /api/v1/auditorias/filter-options` no ejecuta JOIN con `AspNetUsers`; extrae `EntityName` y `Operation` directamente de la tabla `Auditorias`. El tipo `AuditoriaFilterOptions` (`SGV.Contracts/Auditoria/AuditoriaFilterOptions.cs`) es un `sealed record` con únicamente `IReadOnlyList<string> EntityNames` y `IReaditorList<string> Operations`; por construcción física no puede arrastrar campos prohibidos al wire (D-2 reforzado).
