...
### Requisito: Catálogo Habilidades Administrable
...
#### Escenario: Persistir Habilidad activa
...
#### Escenario: Baja lógica sin eliminación física
...
#### Escenario: Desactivar Habilidad referenciada
...
#### Escenario: Preservar estrategia MySQL para unicidad activa
...
#### Escenario: Duplicado activo por persona y puesto
...
#### Escenario: Duplicado activo por puesto
...
### Requisito: Tipo de asignación enumerado en ocupaciones
...
### Requisito: Personas Administrables Persistidas
...
#### Escenario: Persistir persona activa
...
#### Escenario: Baja lógica de persona
...
#### Escenario: Reactivar persona persistida
...
### Requisito: Unicidad Activa de Persona en MySQL
...
#### Escenario: Rechazar legajo activo duplicado
...
#### Escenario: Rechazar email activo duplicado
...
#### Escenario: Rechazar documento activo duplicado
...
### Requisito: Vínculo Obligatorio Usuario-Persona
...
### Requisito: Seed de Roles Fijos de Identity
...
### Requisito: Catálogo `TiposDocumento` con FK `OnDelete(Restrict)`
...
### Requisito: Navegación `Persona.TipoDocumento` y FK configurada
...
### Requisito: `ActiveDocumentoUnique` reconstruido con la nueva fórmula
...
### Requisito: Migración histórica backfill de `TipoDocumento` (string) a `TipoDocumentoId` (FK)
...
### Requisito: `PersonaConfiguracion` reemplaza `TipoDocumento` string por `TipoDocumentoId` FK
...
### Requisito: Coincidencia de tipo entre columna generada y columna fuente
...
#### Escenario: Inserción de OcupacionEntity activa persiste el Guid como string
...
#### Escenario: Duplicado activo por Puesto se rechaza por unicidad, no por truncado
...
#### Escenario: El modelo EF declara `ActivePuestoIdUnique` como `string`/`char(36)`
...
#### Escenario: Migración con purga defensiva pre-alter y forward-only
...
### Requisito: Catálogo `CategoriasHabilidad` con FK `OnDelete(Restrict)` (REQ-CAT-01)
...
#### Escenario: Estructura de la tabla `CategoriasHabilidad`
...
#### Escenario: Seed crea exactamente 4 filas
...
### Requisito: `Habilidades.CategoriaId` FK opcional con `OnDelete(Restrict)` (REQ-CAT-02)
...
#### Escenario: Esquema post-migración no contiene columna `Categoria`
...
#### Escenario: Enforcement de la FK con `OnDelete(Restrict)`
...
### Requisito: Migración fail-loud con pre-flight de strings sucios (REQ-CAT-04 — variante opt-in relajada)
...
#### Escenario: Backfill limpio mapea categorías semilla a GUIDs
...
#### Escenario: Valores sucios caen a `NULL` con auditoría
...
#### Escenario: Drop de la columna legacy tras backfill
