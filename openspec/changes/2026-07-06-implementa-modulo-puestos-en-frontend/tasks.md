...
- [x] **3B.1** RED `PuestoEditPageTests` ≥8: anónimo redirige, render 3 campos prellenados, no encontrado recuperable, **`Get_Edit_HtmlRenderizado_NoContieneCodigoUnidadOrganizativaNiCargo`** (RED obligatorio), POST éxito → PRG Details, POST 400 FieldErrors, POST 409.
- [x] **3B.2** GREEN `Edit.cshtml(.cs)` `IsEdit=true`, `OnPostAsync` → `UpdateAsync`.
- [x] **3B.3** REFACTOR+VERIFY tokens `>Crear<` ausentes en `Edit.cshtml`.
