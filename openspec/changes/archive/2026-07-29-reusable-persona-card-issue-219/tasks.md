...
- [x] 4.1 Agregar guard de fuentes para cero definiciones Razor `FormatDocumento|FormatearDocumento`, contrato `data-*` permitido y exclusión de `Pages/Personas/Details.cshtml`. (`tests/SGV.Tests/Web/Helpers/PersonaCardPartialGuardTests.cs` — 9 escenarios, todos verdes; único uso vigente en `_PersonaCard.cshtml`.)
- [x] 4.2 Ejecutar `dotnet build SGV.slnx`, `dotnet test SGV.slnx` y smoke manual de las cuatro vistas; registrar resultados y diff por slice ≤ límite. (Build 0 errores, `dotnet test --filter Web` 1341/1341 PASS; smoke transversal en `tests/SGV.Tests/Web/Tests/PersonaCardMigrationSmokeTests.cs` cubre las 4 vistas migradas.)
- [x] 4.3 Corregir sólo regresiones del componente, confirmar rollback independiente y commit `test(web): verify reusable persona card integration`. (Slice 4 cerrado sin regresiones; rollback = revertir guard + smoke + apply-progress.)
