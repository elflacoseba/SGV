# Delta para `jwt-signing-key-validation`

> Contrato de arranque de `JwtOptions.SigningKey`: presencia obligatoria y ≥32 bytes UTF-8, fail-loud vía `ValidateOnStart`. Reemplaza el default hardcodeado. `Issuer`/`Audience` fuera de alcance.
