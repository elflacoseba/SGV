...
| D3b | **Corte inmediato** (Web): `CookiePrincipalRevalidator` registrado en `AddCookie.Events.OnValidatePrincipal`. Cliente HTTP dedicado (no `UserManager` directo) — `SGV.Web` no referencia `SGV.Api`/`SgvDbContext`. El cliente consulta endpoint del API con JWT bearer del `AuthenticationProperties` y delega la decisión. Fail-open en 5xx: preserva cookie temporalmente, log warning. `ApiBearerTokenHandler` propaga el rechazo. | Mantiene la frontera arquitectónica (`SGV.Web` shell, `SGV.Api` composition root). Fail-open evita DoS por API degradada. Coste: 1 HTTP extra/request autenticada (mitigable con cache Q2, diferido). |
