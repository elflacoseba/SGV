...
(Previously: no existía sort; los enlaces de paginación no preservaban sort.)
...
- `openspec/changes/2026-07-31-ajustes-listado-auditoria/specs/auditoria-sort/spec.md` (esta spec).

## MODIFIED Files

- `src/SGV.Infraestructura/Persistencia/AuditoriaServicioConsulta.cs` — switch dinámico de sort.
- `src/SGV.Web/Pages/Auditorias/Index.cshtml` — headers `<th>` ordenables con indicadores.
- `src/SGV.Web/Pages/Auditorias/Index.cshtml.cs` — `Sort` bindeado, `BuildSortRouteValues`, `BuildPagedRouteValues` preserva `sort`.
- `src/SGV.Web/Integration/Auditoria/IAuditoriaApiClient.cs` — `BuildQueryUri` propaga `sort`.
- `src/SGV.Web/Integration/Auditoria/AuditoriaApiClient.cs` — implementar propagación.