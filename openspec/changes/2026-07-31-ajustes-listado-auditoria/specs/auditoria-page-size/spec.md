...
- `openspec/changes/2026-07-31-ajustes-listado-auditoria/specs/auditoria-page-size/spec.md` (esta spec).

## MODIFIED Files

- `src/SGV.Web/Pages/Auditorias/Index.cshtml` — `<select>` de `pageSize`, enlaces de paginación/orden lo preservan.
- `src/SGV.Web/Pages/Auditorias/Index.cshtml.cs` — `PageSize` bindeado, normalización al default, `BuildPagedRouteValues`/`BuildSortRouteValues` incluyen `pageSize`.
- `src/SGV.Web/Integration/Auditoria/IAuditoriaApiClient.cs` — `BuildQueryUri` propaga `pageSize`.
