...
/// <summary>
/// Maps a non-success <see cref="PuestoCommandResult"/> into a
...
public static class PuestoPostResultMapper
{
    ...
    /// returns <c>true</c> when <see cref="PuestoCommandResult.FieldErrors"/>
    /// was applied (per-field errors prefixed with
    /// <see cref="PuestoFormKeys.InputPrefix"/>). Returns <c>false</c>
    ...
    public static bool TryMap(PuestoCommandResult? result, ModelStateDictionary modelState)
    {
        ...
        {
            PuestoFormHelpers.ApplyFieldErrorsToModelState(modelState, fieldErrors);
            return true;
        }
