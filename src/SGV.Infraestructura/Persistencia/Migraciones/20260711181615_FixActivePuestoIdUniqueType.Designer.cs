// <auto-generated />
//
// HAND-WRITTEN: This Designer.cs was authored manually because `dotnet ef
// migrations add` failed with NullReferenceException when combining
// HasColumnType + HasComputedColumnSql + string property in EF Core 9 +
// Pomelo 9 (see OccupacionConfiguracion.cs for the active workaround).
//
// If the NRE is fixed in a future EF Core / Pomelo release, re-run
// `dotnet ef migrations add FixActivePuestoIdUniqueType --project
// src/SGV.Infraestructura --startup-project src/SGV.Infraestructura` and
// merge the regenerated file. The BuildTargetModel snapshot below must
// reflect the model state at the time this migration was authored.
//
// Verified against `dotnet test --filter FullyQualifiedName~ModeloPersistenciaTests`
// and `dotnet test --filter FullyQualifiedName~Migraciones_ScriptIdempotenteNoGeneraDDL`
// on CI with MySQL 8.
using System;
...
namespace SGV.Infraestructura.Persistencia.Migraciones
{
    ...
    [Migration("20260711181615_FixActivePuestoIdUniqueType")]
    partial class FixActivePuestoIdUniqueType
    {
        /// <inheritdoc />
